...
def check_move(grid, x1, y1, x2, y2):
    ...
    False
    >>> # check out of grid negative
    >>> grid = [[None, 's', 'r'], [None, None, None]]
    >>> check_move(grid, 1, 0, -4, 0)  # left ok
    False
    """
    if x1 < 0:
        x1 = len(grid[0]) + x1
    ...
    if y2 > (len(grid) - 1) or x2 > (len(grid[0]) - 1):
        return False

    if grid[y2][x2] != None:
        return False

    if y2 > y1 and x2 != x1 and grid[y2-1][x2] != None:
        return False
...
def do_brownian(grid, x, y, brownian):
    ...
    """
    if grid[y][x] == 's':
        if random.randrange(100) < brownian:
